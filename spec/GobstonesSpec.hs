...
describe "gobstones" $ do
    ...
    it "translates simple variable assignment" $ do
      ...
      code `shouldBe` EntryPoint (Variable "x" MuTrue)
    ...
    it "translates simple variable assignment" $ do
      ...
      code `shouldBe`  EntryPoint (Variable "x" (Application (Reference "&&") [Application Equal [MuTrue,MuNumber 2.0],Application NotEqual [Reference "x",Reference "t"]]))
    ...
    it "translates simple procedure declaration and application  with a parameter" $ do
      ...
      code `shouldBe` Sequence [
                        EntryPoint (Application (Reference "F") [MuTrue]),
                        SimpleProcedure "F" [VariablePattern "parameter"] MuNull]

    it "translates conditional declaration" $ do
      let code = gbs "program{if(True){}}"

      code `shouldBe` EntryPoint (If MuTrue MuNull MuNull)

    it "translates conditional declaration" $ do
      let code = gbs "program{if(True){x := 1}}"

      code `shouldBe` EntryPoint (If MuTrue (Variable "x" (MuNumber 1.0)) MuNull)

    it "translates while declaration" $ do
      let code = gbs "program{while(True){}}"

      code `shouldBe` EntryPoint (While MuTrue MuNull)

    it "translates while declaration" $ do
      let code = gbs "program{while(True){x := 1}}"

      code `shouldBe` EntryPoint (While MuTrue (Variable "x" (MuNumber 1.0)))
