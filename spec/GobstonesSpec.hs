...
describe "translateProgramGobstonesToMulangExpression" $ do
    ...
    it "translate simple variable assignment" $ do
      ...
      gobstonesAstNumber `shouldBe` VariableDeclaration "x" (MuNumber 1.0)

      gobstonesAstColour `shouldBe` VariableDeclaration "x" (MuSymbol "Verde")

      gobstonesAstBool `shouldBe` VariableDeclaration "x" (MuBool True)

      gobstonesAstDirection `shouldBe` VariableDeclaration "x" (MuSymbol "Este")

      gobstonesAstVariable `shouldBe` VariableDeclaration "x" (Variable "y")

      gobstonesAstFunctionCall `shouldBe` VariableDeclaration "x" (Application (Variable "f") [MuNumber 2.0])

      gobstonesAstSimpleExpression `shouldBe` VariableDeclaration "x" (Application (Variable "&&") [Variable "z",Variable "y"])

      gobstonesAstComposeExpression `shouldBe`  VariableDeclaration "x" (Application (Variable "&&") [Application Equal [MuBool True,MuNumber 2.0],Application NotEqual [Variable "x",Variable "t"]])
    ...
    it "translate conditional declaration" $ do
      ...
      gobstonesAstIfWithBody `shouldBe` If (MuBool True) (VariableDeclaration "x" (MuNumber 1.0)) MuNull
    ...
    it "translate while declaration" $ do
      ...
      gobstonesAstWhileWithBody `shouldBe` While (MuBool True) (VariableDeclaration "x" (MuNumber 1.0))
    ...
    it "translate switch declaration" $ do
      ...
      gobstonesAst `shouldBe`  Switch (MuNumber 2.0) [(MuNumber 2.0,VariableDeclaration "x" (MuNumber 2.0))]
    ...
    it "translate repeat declaration" $ do
      ...
      gobstonesAst `shouldBe`  Repeat (MuNumber 2.0) (VariableDeclaration "x" (MuNumber 2.0))
    ...
    it "translate a complete program" $ do
      ...
      gobstonesAst `shouldBe`  Sequence [Application (Variable "F") [MuSymbol "Verde"],Application (Variable "G") [MuSymbol "Negro",MuSymbol "Verde"],Application (Variable "X") [MuSymbol "Este"],VariableDeclaration "y" (Application (Variable "f") [MuBool False]),ProcedureDeclaration "F" [Equation [VariablePattern "x"] (UnguardedBody (Sequence [Application (Variable "Poner") [Variable "x"],Application (Variable "Sacar") [Variable "x"]]))],ProcedureDeclaration "G" [Equation [VariablePattern "n1",VariablePattern "n2"] (UnguardedBody (Sequence [VariableDeclaration "x" (Variable "n1"),VariableDeclaration "z" (Variable "n2"),While (MuBool True) (VariableDeclaration "x" (Variable "n1")),Switch (Variable "dir") [(MuSymbol "Sur",Application (Variable "Poner") [MuNumber 3.0]),(MuSymbol "Este",Application (Variable "Poner") [MuNumber 3.0]),(MuSymbol "Oeste",Application (Variable "Poner") [MuNumber 3.0]),(MuSymbol "Norte",Application (Variable "Poner") [MuNumber 3.0])]]))],ProcedureDeclaration "X" [Equation [VariablePattern "dir"] (UnguardedBody (Application (Variable "Mover") [Variable "dir"]))],FunctionDeclaration "f" [Equation [VariablePattern "bool"] (UnguardedBody (Sequence [VariableDeclaration "g" (MuNumber 2.0),If (MuBool False) (VariableDeclaration "resultado" (MuBool True)) (VariableDeclaration "resultado" (Variable "resultado")),Return (Variable "resultado")]))]]
