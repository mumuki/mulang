...
describe "translateProgramGobstonesToMulangExpression" $ do
    ...
    it "translate simple procedure Call" $ do
      ...
      gobstonesAst `shouldBe` Sequence [Application (Variable "F") []]
    ...
    it "translate simple procedure Application " $ do
      ...
      gobstonesAstWithPoner `shouldBe` Sequence [Application (Variable "Poner") [MuSymbol "Verde"]]

      gobstonesAstWithSacar `shouldBe` Sequence [Application (Variable "Sacar") [MuSymbol "Verde"]]

      gobstonesAstWithMover `shouldBe` Sequence [Application (Variable "Mover") [MuSymbol "Este"]]
    ...
    it "translate simple variable assignment" $ do
      ...
      gobstonesAstNumber `shouldBe` Sequence [VariableAssignment "x" (MuNumber 1.0)]

      gobstonesAstColour `shouldBe` Sequence [VariableAssignment "x" (MuSymbol "Verde")]

      gobstonesAstBool `shouldBe` Sequence [VariableAssignment "x" (MuBool True)]

      gobstonesAstDirection `shouldBe` Sequence [VariableAssignment "x" (MuSymbol "Este")]

      gobstonesAstVariable `shouldBe` Sequence [VariableAssignment "x" (MuString "y")]

      gobstonesAstFunctionCall `shouldBe` Sequence [VariableAssignment "x" (Application (Variable "f") [MuNumber 2.0])]

      gobstonesAstSimpleExpression `shouldBe` Sequence [VariableAssignment "x" (Application (Variable "&&") [MuString "z",MuString "y"])]

      gobstonesAstComposeExpression `shouldBe` Sequence [VariableAssignment "x" (Application (Variable "&&") [Application (Variable "==") [MuBool True,MuNumber 2.0],Application (Variable "!=") [MuString "x",MuString "t"]])]
    ...
    it "translate conditional declaration" $ do
      ...
      gobstonesAst `shouldBe` Sequence [If (MuBool True) MuNull MuNull]

      gobstonesAstIfWithBody `shouldBe` Sequence [If (MuBool True) (Sequence [VariableAssignment "x" (MuNumber 1.0)]) MuNull]
    ...
    it "translate while declaration" $ do
      ...
      gobstonesAst `shouldBe` Sequence [While (MuBool True) MuNull]

      gobstonesAstWhileWithBody `shouldBe` Sequence [While (MuBool True) (Sequence [VariableAssignment "x" (MuNumber 1.0)])]
    ...
    it "translate switch declaration" $ do
      ...
      gobstonesAst `shouldBe`  Sequence [Switch (MuNumber 2.0) [(MuNumber 2.0,Sequence [VariableAssignment "x" (MuNumber 2.0)])]]
    ...
    it "translate repeat declaration" $ do
      ...
      gobstonesAst `shouldBe`  Sequence [Repeat (MuNumber 2.0) (Sequence [VariableAssignment "x" (MuNumber 2.0)])]
